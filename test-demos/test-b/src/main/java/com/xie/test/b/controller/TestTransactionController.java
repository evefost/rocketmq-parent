...
public class TestTransactionController implements ApplicationContextAware {
    ...
    public String addUser3() {
        count++;
        transactionService.addUser3(count);
        return "addUser3";
    }
    ...
    public String transastion() throws InterruptedException, RemotingException, MQClientException, MQBrokerException {
        ...
        User user =new User();
        user.setName("测试布式事件");
        Message message = new Message("656555555555","addUser",null,JSON.toJSONString(user).getBytes());
...
//    };
    ...
    @RequestMapping(value = "/concurrentTest", method = RequestMethod.GET)
    public String concurrentTest(Integer threads,Integer msgCount) {
        if(threads == null){
            threads = 1;
        }
        if(msgCount == null){
        ...
        String ss = "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhssss";
...
//        System.out.println("消息大小:"+bytes.length);
        ...
        count++;
        msgtotal = new AtomicInteger(0);
        System.out.println("前端每个请开"+threads+" 线程发消息:"+msgCount);
        CountDownLatch countDownLatch = new CountDownLatch(threads);
        CountDownLatch finish = new CountDownLatch(threads);

        for(int i=0;i<threads;i++){
            Integer finalMsgCount = msgCount;
            new Thread(){
                @Override
                public void run() {
                    try {
                        ...
                        for(int j = 0; j< finalMsgCount; j++){
                            msgtotal.incrementAndGet();
//                            interFaceA.addUser(user);
                            SourceEvent event = new SourceEvent(this, user);
                            event.setTopic("TopicAsssss777");
                            publisher.publishEvent(event, new SendMsgCallback() {
                                @Override
        ...
        System.out.println("消息发送完总数:"+msgtotal.intValue()+" 失败数: "+innerMessageSender.getFailueCount()+"   失败率:"+innerMessageSender.getFailurePercentage());
        return "addUser2:"+count;
    }

    @RequestMapping(value = "/publisSycMsg", method = RequestMethod.GET)
    public SendMsgResult publisSycMsg() {
        count++;
        User user = new User();
        user.setAge(count);
        user.setName("publisher publishEvent发布的的消息");
        SourceEvent event = new SourceEvent(this, "publisSycMsg","publisher", user);
        SendMsgResult sendResult = publisher.publishSycEvent(event);
        return sendResult;
    }

    @RequestMapping(value = "/publisAsycMsg", method = RequestMethod.GET)
    public SendMsgResult publisAsycMsg() {
        count++;
        User user = new User();
        user.setAge(count);
        user.setName("publisher publishEvent发布的的消息");
